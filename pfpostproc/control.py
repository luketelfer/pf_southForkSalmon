...
def zarr_write_ds(ds):
    ds.to_zarr(
        ...
        mode='a',
        group=f'control/{scenario}/B{batch}',
        compute=True,
        consolidated=False);
...
def pf_workflow(rundir,runname,domain,tcoords):
    ...
    pf['t'] = tcoords
    zarr_write_ds(pf.expand_dims({'b':[bval],'c':[scenario]}))
    return pf
...
def calc_workflow(pf,domain,tcoords):
    ...
    calcs['t'] = tcoords
    zarr_write_ds(calcs.expand_dims({'b':[bval],'c':[scenario]}))
...
def clm_workflow(rundir,runname,domain,tcoords):
    ...
    clm['t'] = tcoords
    zarr_write_ds(clm.expand_dims({'b':[bval],'c':[scenario]}))
